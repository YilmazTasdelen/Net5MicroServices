...
namespace FreeCourse.Services.Basket
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.Configure<RedisSetings>(Configuration.GetSection("RedisSettings"));
            services.AddScoped<ISharedIdentityService, SharedIdentityService>();
            services.AddScoped<IBasketService, BasketService>();
            services.AddSingleton<RedisService>(sp=> 
            {
                var redisSettings = sp.GetRequiredService<IOptions<RedisSetings>>().Value;
                var redis = new Services.RedisService(redisSettings.Host, redisSettings.Port);
                redis.Connect();
                return redis;
            });
